# Project Service Architecture Blueprint
...
architecture for Project Service.
...
```text
One Project Service process
└── One Project Service PostgreSQL database
    ├── catalog module
    ├── source module
    ├── release module
    ├── interaction module
...
|---|---|---|
| Deployment | One Project Service application | The features share a lifecycle and the codebase is still small. |
| Data | One database owned only by Project Service | No other service may query or write its tables directly. |
...
Use direct subpackages of `com.philia.projectservice` as business modules. A
...
package com.philia.projectservice.release;
```
...
void verifiesModuleBoundaries() {
    ApplicationModules.of(ProjectServiceApplication.class).verify();
}
```
...
Keep one Project Service changelog because there is one service-owned database.
...
- Its workload harms the rest of Project Service despite local isolation and
  tuning.
...
should not share Project Service tables.
